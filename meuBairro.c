...
typedef struct v{ /* Cada vertice tem um ponteiro para uma lista de arestas incidentes nele */
	...
	int cor;
	Arest *prim;
	int pred; // **NOVO: Predecessor para reconstruir o caminho**
}Vert;
...
// Protótipos de funções para cálculo de distâncias e caminhos
void calculaDistancias(Vert G[], int ordem, int origem, int dist[]);
int distanciaAteLocalidade(int dist[], Localidade loc, int *vertice_mais_proximo); // **NOVO: Passa o vértice mais próximo**
void imprimeCaminho(Vert G[], int origem, int destino); // **NOVO: Função para imprimir o caminho**
...
void criaGrafo(Vert **G, int ordem){
	...
	*G= (Vert*) malloc(sizeof(Vert)*ordem);
	for(i=0; i<ordem; i++){
		...
		(*G)[i].cor= BRANCO;
		(*G)[i].prim= NULL;
		(*G)[i].pred= -1; // **Inicializa predecessor como -1 (NULO)**
	}
}
...
void calculaDistancias(Vert G[], int ordem, int origem, int dist[]) {
	...
	for (int i = 0; i < ordem; i++) {
		dist[i] = INT_MAX;
		G[i].pred = -1; // **Reinicia predecessor**
	}
	dist[origem] = 0;
	G[origem].pred = origem; // **A origem aponta para si mesma ou pode ser -1**
	fila[fim++] = origem;
	while (ini < fim) {
		int v = fila[ini++];
		Arest *a = G[v].prim;
		while (a != NULL) {
			...
			if (dist[u] == INT_MAX) {
				dist[u] = dist[v] + 1;
				G[u].pred = v; // **REGISTRA O PREDECESSOR**
				fila[fim++] = u;
			}
			a = a->prox;
		}
...
// Retorna a menor distância a uma localidade e o vértice mais próximo
int distanciaAteLocalidade(int dist[], Localidade loc, int *vertice_mais_proximo) {
	if (loc.v2 == -1) {
		*vertice_mais_proximo = loc.v1;
		...
		if (d1 <= d2) {
			*vertice_mais_proximo = loc.v1;
			if (d1 == INT_MAX) return INT_MAX;
			return d1 + 1; // Distância até a aresta (v1, v2)
		} else {
			*vertice_mais_proximo = loc.v2;
			if (d2 == INT_MAX) return INT_MAX;
			return d2 + 1; // Distância até a aresta (v1, v2)
		}
	}
}

// **NOVA FUNÇÃO: Imprime o caminho recursivamente do destino à origem**
void imprimeCaminho(Vert G[], int origem, int destino) {
	if (destino == origem) {
		printf("v%d", origem);
	} else if (G[destino].pred == -1) {
		printf("Caminho inacessível ou erro na busca.");
	} else {
		imprimeCaminho(G, origem, G[destino].pred);
		printf(" -> v%d", destino);
	}
}
...
int main(int argc, char *argv[]) {
	...
	// ... (manutenção das suas chamadas a acrescentaAresta)
	...
	// ... (manutenção das suas chamadas a strcpy e atribuições)
	...
	printf("\nDistancias em quadras a partir da Minha Casa (v%d):\n", origem);
	for (int i=0;i<qtdLocal;i++){
		int vertice_de_chegada = -1; // Vértice do grafo que minimiza a distância
		int d = distanciaAteLocalidade(dist,locais[i], &vertice_de_chegada);
		
		printf(" - %s: ",locais[i].nome);
		if (d==INT_MAX){
			printf("INACESSIVEL\n");
		} else {
			printf("%d quadras. Caminho: ",d);
			if (locais[i].v2 == -1) {
				// Localidade está em um único vértice
				imprimeCaminho(G, origem, locais[i].v1);
			} else {
				// Localidade está em uma aresta (v1, v2). O caminho vai até o vértice mais próximo.
				imprimeCaminho(G, origem, vertice_de_chegada);
				printf(" -> ");
				// Adiciona o outro vértice/localidade final
				if (vertice_de_chegada == locais[i].v1) {
					printf("v%d (Local) \n", locais[i].v2);
				} else {
					printf("v%d (Local) \n", locais[i].v1);
				}
